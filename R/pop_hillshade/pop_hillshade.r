...
head(pop_df)
summary(pop_df$npl_ppp_2020_constrained)
pop_df$npl_ppp_2020_constrained[pop_df$npl_ppp_2020_constrained <= 0.1] <- NA
...
p <- ggplot() +
  ...
  annotation_scale(
    location = "bl", pad_y = unit(50, "mm"),
    height = unit(2, "mm")
  ) +
  coord_sf(expand = FALSE) +
  # 5) typography & layout
  labs(
    title = "Nepal · Population (2020)",
    subtitle = "WorldPop 100m constrained grid",
    caption = "Data: WorldPop · SRTM via elevatr | Design: Jackson Mbugua"
  ) +
  theme(
